#!/usr/bin/env python3
"""
Prompt templates for Claude Code SDK operations.

This module contains prompt templates used by the convert.py script for
HTML to Markdown conversion and conversion verification tasks.

The module provides both object-oriented prompt classes and backward-compatible
string constants for existing code.
"""
...
class PromptTemplate(ABC):
    ...
    Provides common functionality for template formatting, parameter validation,
    and error handling.
    """
    ...
    def validate_parameters(self, params: Dict[str, Any], required: list) -> None:
        """
...
class ConversionPrompt(PromptTemplate):
    """
    Prompt templates for HTML to Markdown conversion operations.

    Handles both planning and execution phases of the conversion process.
    """

    def get_planning_prompt(self, input_path: str, output_path: str) -> str:
        """
        Get the planning phase prompt for conversion.
        ...
        return self.format_template(self._PLANNING_TEMPLATE,
                                   input_path=input_path,
                                   output_path=output_path)

    def get_execution_prompt(self, output_path: str) -> str:
        """
        Get the execution phase prompt for conversion.
        ...
        """
        self.validate_parameters({'output_path': output_path}, ['output_path'])

        return self.format_template(self._EXECUTION_TEMPLATE, output_path=output_path)

    _PLANNING_TEMPLATE = """STEP 1: PLANNING PHASE

You need to convert an HTML file to Markdown. First, read the input file and create a plan.

<input_file_path>{input_path}</input_file_path>
<output_file_path>{output_path}</output_file_path>

Please:
1. Use the Read tool to read the HTML content from: {input_path}
2. Analyze the HTML structure and content
3. Create a detailed plan using TodoWrite for the conversion process

Your plan should include:
1. Analysis of the HTML structure and content
2. Identification of elements to remove (navigation, headers, footers, etc.)
3. Identification of main content areas to preserve
4. Strategy for converting HTML elements to Markdown
5. Quality checks to perform after conversion

Use TodoWrite to create a structured task list for this conversion process."""

    _EXECUTION_TEMPLATE = """STEP 2: EXECUTION PHASE

Now execute the plan you created. Convert the HTML content to clean, well-formatted Markdown following your plan.

Requirements:
- Follow the plan you created in Step 1
- Use the Read tool again if needed to re-examine the HTML content
- Remove all navigation elements, headers, footers, sidebars, and advertisements
- Focus only on the main content area
- Preserve the document structure with proper heading hierarchy
- Convert HTML links to markdown format
- Handle images with proper alt text
- Clean up any redundant whitespace
- Ensure the output is valid markdown syntax
- Use Write tool to save the converted content to: {output_path}
- Update your TodoWrite tasks as you complete each step

Execute your plan and save the converted markdown content."""


class VerificationPrompt(PromptTemplate):
    """
    Prompt templates for conversion verification operations.

    Handles both planning and execution phases of the verification process.
    """

    def get_planning_prompt(self, original_path: str, converted_path: str,
                           original_format: str, converted_format: str) -> str:
        """
        Get the planning phase prompt for verification.
        ...
        self.validate_parameters(params, list(params.keys()))

        return self.format_template(self._PLANNING_TEMPLATE, **params)

    def get_execution_prompt(self, original_path: str, converted_path: str,
                            original_format: str, converted_format: str) -> str:
        """
        Get the execution phase prompt for verification.
        ...
        return self.format_template(self._EXECUTION_TEMPLATE, **params)

    _PLANNING_TEMPLATE = """STEP 1: PLANNING PHASE

You need to verify the conversion between {original_format} and {converted_format} formats.

<original_file_path>{original_path}</original_file_path>
<converted_file_path>{converted_path}</converted_file_path>

Please:
1. Use the Read tool to examine both files: {original_path} and {converted_path}
2. Create a detailed verification plan using TodoWrite

Your plan should include:
1. Analysis of both original and converted files structure and content
2. Comparison strategy for content integrity
3. Formatting accuracy checks specific to {converted_format}
4. Structural preservation verification
5. Completeness assessment methodology
6. Quality metrics to evaluate
7. Specific checks for headings, links, code blocks, lists, tables
8. Final verification report generation

Use TodoWrite to create a structured task list for this verification process."""

    _EXECUTION_TEMPLATE = """STEP 2: EXECUTION PHASE

Now execute the verification plan you created. Perform comprehensive verification of the conversion.

Requirements:
- Follow the plan you created in Step 1
- Use Read tool to re-examine both files if needed: {original_path} and {converted_path}
- Perform detailed comparison between {original_format} and {converted_format}
- Check content integrity, formatting accuracy, structural preservation
- Verify completeness and readability
- Update your TodoWrite tasks as you complete each verification step
- Generate a verification report if needed using Write tool

At the end, return ONLY "PASS" if the conversion is completely correct, or provide a detailed error message describing specific issues found."""


class PromptGenerator(PromptTemplate):
    """
    Prompt templates for metaprompt generation and variable management.

    Handles prompt generation and floating variable removal operations.
    """

    def get_generation_prompt(self, task: str) -> str:
        """
        Get the metaprompt generation prompt.

        Args:
            task: The task description for prompt generation

        Returns:
            Formatted generation prompt
        """
        self.validate_parameters({'task': task}, ['task'])

        return self.format_template(self._GENERATION_TEMPLATE, TASK=task)

    def get_variable_removal_prompt(self, prompt: str) -> str:
        """
        Get the floating variable removal prompt.

        Args:
            prompt: The prompt to process for variable removal

        Returns:
            Formatted variable removal prompt
        """
        self.validate_parameters({'prompt': prompt}, ['prompt'])

        return self.format_template(self._VARIABLE_REMOVAL_TEMPLATE, PROMPT=prompt)
...
# Create prompt class instances for easy access
conversion_prompt = ConversionPrompt()
verification_prompt = VerificationPrompt()
metaprompt_generator = PromptGenerator()


# Backward compatibility: Maintain original constant names
# Conversion prompt templates

def get_converter_planning_prompt(input_path: str, output_path: str) -> str:
    """Get the planning prompt for HTML to Markdown conversion.

    This function provides backward compatibility with the original constant.
    For new code, prefer using ConversionPrompt().get_planning_prompt().
    ...
    """
    return conversion_prompt.get_planning_prompt(input_path, output_path)

def get_converter_execution_prompt(output_path: str) -> str:
    """Get the execution prompt for HTML to Markdown conversion.

    This function provides backward compatibility with the original constant.
    For new code, prefer using ConversionPrompt().get_execution_prompt().
    ...
    """
    return conversion_prompt.get_execution_prompt(output_path)

def get_verifier_planning_prompt(original_path: str, converted_path: str,
                                original_format: str, converted_format: str) -> str:
    """Get the planning prompt for conversion verification.

    This function provides backward compatibility with the original constants.
    For new code, prefer using VerificationPrompt().get_planning_prompt().
    ...
    Returns:
        Formatted planning prompt string
    """
    return verification_prompt.get_planning_prompt(
        original_path, converted_path, original_format, converted_format
    )

def get_verifier_execution_prompt(original_path: str, converted_path: str,
                                 original_format: str, converted_format: str) -> str:
    """Get the execution prompt for conversion verification.

    This function provides backward compatibility with the original constants.
    For new code, prefer using VerificationPrompt().get_execution_prompt().
    ...
    Returns:
        Formatted execution prompt string
    """
    return verification_prompt.get_execution_prompt(
        original_path, converted_path, original_format, converted_format
    )

def get_generate_prompt_prompt(task: str) -> str:
    """Get the metaprompt generation prompt.

    This function provides backward compatibility with the original constant.
    For new code, prefer using PromptGenerator().get_generation_prompt().
    ...
    """
    return metaprompt_generator.get_generation_prompt(task)

def get_remove_floating_variables_prompt(prompt: str) -> str:
    """Get the floating variable removal prompt.

    This function provides backward compatibility with the original constant.
    For new code, prefer using PromptGenerator().get_variable_removal_prompt().

    Args:
        prompt: The prompt to process for variable removal

    Returns:
        Formatted variable removal prompt string
    """
    return metaprompt_generator.get_variable_removal_prompt(prompt)


# Backward compatibility: Original constant names
# These maintain the original interface for existing code
CONVERTER_PLANNING_PROMPT = conversion_prompt._PLANNING_TEMPLATE
CONVERTER_EXECUTION_PROMPT = conversion_prompt._EXECUTION_TEMPLATE
VERIFIER_PLANNING_PROMPT = verification_prompt._PLANNING_TEMPLATE
VERIFIER_EXECUTION_PROMPT = verification_prompt._EXECUTION_TEMPLATE
GENERATE_PROMPT_PROMPT = metaprompt_generator._GENERATION_TEMPLATE
REMOVE_FLOATING_VARIABLES_PROMPT = metaprompt_generator._VARIABLE_REMOVAL_TEMPLATE
