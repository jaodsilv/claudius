...
- **Output styles** replace the base system prompt entirely (except for the core tool descriptions)
- **CLAUDE.md** appends additional context and instructions to the existing system prompt
- **—append-system-prompt** is a one-time addition to the system prompt for a single conversation
...
Output styles and [agents](/en/docs/claude-code/agents) serve different purposes:

- **Output styles** change how Claude responds and behaves globally across all interactions
- **Agents** are task-specific tools that can be invoked to handle particular workflows or processes

Think of output styles as changing Claude's "personality" or approach, while agents are specialized tools for specific tasks.
...
- **Output styles** modify the fundamental behavior and response style of Claude Code
- **Custom slash commands** provide quick access to frequently used prompts or workflows
