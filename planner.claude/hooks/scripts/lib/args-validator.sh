#!/bin/bash
# Argument validation for planner plugin
...
get_positional_arg() {
  ...
  for part in "${parts[@]}"; do
    ...
    if [[ "$part" =~ ^- ]]; then
      case "$part" in
        --use-brainstorm|--depth|--output|--mode|--rounds|--framework|--architecture-path|--goal|--prioritization-path|--requirements-path|--roadmap-path|--phases|--horizon)
          skip_next=true
          ;;
      esac
      continue
