...
export _PLUGIN_VALUE_FLAGS=(--worktree --base --files --context --resolve-level --approval-level --format --session-path -l -a -t -m -sc -c --since-commit --single-commit)
...
case "$COMMAND" in
  ...
  comment-to-pr)
    ...
    ;;
  create-issue)
    log_info "Loading issues/create-issue.sh"
    source "$HANDLERS_DIR/issues/create-issue.sh"
    ;;
  ignore)
