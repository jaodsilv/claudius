...
export _PLUGIN_VALUE_FLAGS=(--worktree --base --files --context --resolve-level --approval-level --format --session-path -l -a -t -m -sc -c --since-commit --single-commit --repo --pr)
...
case "$COMMAND" in
  ...
  address-review)
    ...
    ;;
  commit-push)
    log_section "Commit-Push PostToolUse Hook"
