...
- If it's just a number, it's an issue number, store it in `$issueNumber`
- Else if it's a string with at most 2 slashes and no dots or nor spaces, it's a branch name (e.g. `feature/new-feature`), store it in `$branch`
- Else if it's a string without spaces, it's the worktree name, to be placed in `../`, store it in `$dir`.
