...
# Accept worktree as argument, defaulting to current directory
_ARG_WORKTREE="${1:-.}"

# Only set if not already exported by hook dispatcher
if [[ -z "${WORKTREE:-}" ]]; then
  WORKTREE=$(cd "$_ARG_WORKTREE" && pwd)
fi
...
PR_NUMBER=$(gh pr list -R "$REPO_OWNER/$REPO_NAME" --head "$CURRENT_BRANCH" --state open --json number --jq '.[0].number' 2>/dev/null)
if [[ -z "$PR_NUMBER" ]] || [[ "$PR_NUMBER" == "null" ]]; then
  write_no_pr "No open PR found for branch: $CURRENT_BRANCH"
fi
...
ci_runs=$(gh run list -R "$REPO_OWNER/$REPO_NAME" -b "$CURRENT_BRANCH" --json databaseId,url,workflowName 2>/dev/null || echo "[]")
...
# Phase 9c: Compute ciResult
ci_result="SUCCESS"
if [[ "$ci_status" != "[]" ]] && [[ "$ci_status" != "null" ]]; then
  ongoing=$(echo "$ci_status" | jq '[.[] | select(.status != "COMPLETED" and .conclusion != "SKIPPED")] | length')
  failed=$(echo "$ci_status" | jq '[.[] | select(.status == "COMPLETED" and .conclusion != "SUCCESS" and .conclusion != "SKIPPED" and .conclusion != "CANCELLED" and .conclusion != "NEUTRAL" and .conclusion != null and .conclusion != "")] | length')
...
# Use temporary files for large JSON data to avoid Windows command-line length limits
# (Windows has ~8192 char limit for command args, reviews alone can exceed 18K)
TEMP_DIR=$(mktemp -d)
trap 'rm -rf "$TEMP_DIR"' EXIT

echo "$reviews" > "$TEMP_DIR/reviews.json"
echo "$review_threads" > "$TEMP_DIR/review_threads.json"
echo "$latest_minimized_review" > "$TEMP_DIR/latest_minimized_review.json"
echo "$ci_status" > "$TEMP_DIR/ci_status.json"
echo "$filtered_comments" > "$TEMP_DIR/filtered_comments.json"
echo "$historical_comments" > "$TEMP_DIR/historical_comments.json"
...
jq -n \
  ...
  --slurpfile latestReviews "$TEMP_DIR/reviews.json" \
  --slurpfile reviewThreads "$TEMP_DIR/review_threads.json" \
  --slurpfile latestMinimizedReview "$TEMP_DIR/latest_minimized_review.json" \
  --arg latestReviewedCommit "$latest_reviewed_commit" \
  --slurpfile ciStatus "$TEMP_DIR/ci_status.json" \
  --slurpfile latestComments "$TEMP_DIR/filtered_comments.json" \
  --slurpfile historicalComments "$TEMP_DIR/historical_comments.json" \
  ...
  '{
    ...
    latestReviews: $latestReviews[0],
    reviewThreads: $reviewThreads[0],
    latestMinimizedReview: $latestMinimizedReview[0],
    latestReviewedCommit: (if $latestReviewedCommit == "" then null else $latestReviewedCommit end),
    ciStatus: $ciStatus[0],
    ciResult: $ciResult,
    latestComments: $latestComments[0],
    historicalComments: $historicalComments[0],
