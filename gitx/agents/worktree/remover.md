...
tools: Bash(git worktree:*), Bash(git branch:*), Bash(git push:*), Bash(git switch:*), Bash(bash:*), Bash(powershell:*), AskUserQuestion, Skill
model: haiku
---
...
**Critical Step**: Before removing, check for junction points/symlinks that point outside the repository to prevent data loss. Use the skill `gitx:detect-junctions-and-symlinks` to detect junctions.

Check common junction locations (e.g., `data/`, `node_modules/`, `.env` folders)
...
### Step 2: Check for uncommitted, unmerged, unpushed, or untracked changes
