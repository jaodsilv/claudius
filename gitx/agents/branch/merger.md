...
- `--base <branch>`: Base branch to merge from (store in `$base`)
...
- `same_branch`: "Cannot merge: already on the base branch."
- `detached_head`: "Cannot merge: in detached HEAD state."
...
AskUserQuestion:
  ...
  3. "Abort merge" - Run `git merge --abort`
```

For manual mode, show:

- Conflicting files: `git diff --name-only --diff-filter=U`
- Instructions: Edit files, `git add <file>`, `git commit`
