...
tools: Bash(git:*), Bash(gh:*), Task, Read, AskUserQuestion
model: sonnet
---
...
Use the Task tool to run the agent `gitx:pr:metadata-fetcher` with PR_NUMBER as argument if provided, otherwise use an empty prompt.

It will return a JSON object with the following fields:
