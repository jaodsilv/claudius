...
tools: Bash, Read, Write, Edit, Glob, Grep, Task, AskUserQuestion, TaskCreate, TaskGet, TaskList, TaskUpdate, Skill
model: sonnet
...
2. Use Agent tool with `test-automator` agent for test phases
3. Use Agent tool with `code-reviewer` agent for review phases
...
1. **2B.1**: RED Phase - Write failing unit tests
   - Use Agent tool with `test-automator` agent
...
2. **2B.2**: Design solution
   - Use Agent tool for design agent
   - Review design (use Agent tool for review agent)
   - Loop if needed (max 3-5 iterations)
   - [COMPACT: task, worktree, step]

3. **2B.3**: Plan development
   - Use Agent tool for planning agent
   - Review plan (use Agent tool for review agent)
   - Loop if needed (max 3-5 iterations)
   - [COMPACT: task, worktree, step]

4. **2B.4**: GREEN Phase - Minimal implementation
   - Use Agent tool for implementation agent
...
7. **2B.7**: REFACTOR Phase - Improve code quality
   - Use Agent tool with `code-reviewer` agent
   - [COMPACT: task, worktree, step]

### Phase 3: Design Alignment Check

Use Agent tool to launch review agent:
...
Use Agent tool for commit message agent:
...
1. Evaluate refactoring needs (Agent tool)
...
1. Write PR message (Agent tool)
2. Review PR message (Agent tool)
3. Create PR:
...
2. Update roadmap/task list (Agent tool)
3. Remove worktree:
...
3. Re-run RED phase with Agent tool

### If Tests Don't Pass (GREEN Phase)

1. Review failing tests
2. Check implementation completeness
3. Re-run GREEN phase with Agent tool
