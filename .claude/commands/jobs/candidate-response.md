---
description: Generate a response for a candidate to respond to a recruiter that reached out to them
allowed-tools: Read, TodoWrite, Write, LS, Grep, Glob, Edit
argument-hint: resume_filepath: <resume_filepath> input_filepath: <input_filepath> output_path: <output_path>
---
...
- <output_path>Use the Write tool to write into the folder @{{OUTPUT_PATH}}, create the folder if needed</output_path>
- Use the write tool to write intermediate files
- Use the Task tool to launch subagents

## Task

Your task is to launch agents to create a tailored response for a candidate the received an message of a recruiter reaching out to them.
Do that using the Task tool to launch subagents creating a pipeline of agents
...
1. Read the two input files:
   1. Using the Task tool job-description-parser subagent, parse the information from the job description present in the file {{INPUT_FILEPATH}} at the key `.job_description`. Output in <job_description> tag
